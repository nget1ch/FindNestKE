...
export default function HouseDetail() {
  ...
  const { data: house, isLoading: loading, error: fetchError } = useGetHouseByIdQuery(Number(id), { skip: !id });
  ...
  const isOwner = useMemo(() => {
    return user && house && (user.userId === house.landlordId);
  }, [user, house]);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteHouse(Number(houseId)).unwrap();
